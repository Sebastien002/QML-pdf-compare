...
void PdfScreenshotProvider::selectedText(const QString &path, int pageNum, const QPoint &startPos, const QPoint &endPos, const QString &color)
{
    auto fp = findFilePropertiesItemByPath(path);
    ChangeItem *c = createChangeItem(path,pageNum,startPos);
    c->type = TypeSelect;
    c->color = color;
    xDebug << "ZOOM RATIO" <<fp->zoomRatio;
    c->change = endPos/fp->zoomRatio;
    emit updatePage(path,pageNum);
    addRecord(path,c);
}

void PdfScreenshotProvider::addPencil(const QString &path, int pageNum, const QPoint &startPos, const QVariantList& list/*, const QString &color*/)
{
    auto fp = findFilePropertiesItemByPath(path);
    ChangeItem *c = createChangeItem(path,pageNum,startPos);
    c->type = TypePencil;
...
void PdfScreenshotProvider::saveChange(const QString &path)
{
    ...
    for(int i = 0;i < pageCount; i++) { //遍历所有页，取出对应页所有改动，再将其转图片后重新写入
        ...
        writePageChangeToPainter(painter,fp,i,true,path);
        if(i != pageCount - 1 ){
            pdfWriter->newPage();
        }
    }
...
void PdfScreenshotProvider::undo(const QString &path)
{
    ...
    searchedTextItems.removeOne(c->timestamp);
...
void PdfScreenshotProvider::writePageChangeToPainter(QPainter *painter, PdfScreenshotProvider::FileProperties *fp,int pageNum,bool ignoreZoomRatio /*= false*/, const QString &path/* = "" */)
{
    float ratio = 1.0f;
    if(!ignoreZoomRatio) {
        ratio = fp->zoomRatio;
        ...
        if(c->pageNum == pageNum) {
            if(c->type == TypeText) {
               ...
               // qDebug() << ratio << pageSize << textWidth  << c->pos.x()  << x << content;
                ...
                if(text != "" && text != " " && !searchedTextItems.contains(c->timestamp)){
                    g_pdfScreenShotProvider1->searchText(text,path,c->color);
                    searchedTextItems.append(c->timestamp);
                }
                xDebug << text << searchedTextItems;
            } else if(c->type == TypePencil) {
               QPen pen(QColor("#3CBBF9"));
               pen.setWidth(3);
               painter->setPen(pen);
               QPointF startPos(c->pos.x(), c->pos.y());
               foreach (QPointF pos, c->points) {
                    painter->drawLine(startPos, pos);
                    startPos = pos;
               }
           }
        }
...
QImage PdfScreenshotProvider::requestImage(const QString &id, QSize *size, const QSize &requestedSize)
{
    ...
    if(fp->showRecords){
        writePageChangeToPainter(&painter,fp,pageNum.toInt(),false, path);
    }
